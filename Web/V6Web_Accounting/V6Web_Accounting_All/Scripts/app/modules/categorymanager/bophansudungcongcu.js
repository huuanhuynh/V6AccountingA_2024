(function (module) {
    module.controller("BoPhanSuDungCongCuController", BoPhanSuDungCongCuController);
    BoPhanSuDungCongCuController.$inject = ["$scope", "KendoGridOptionsFactory", "$state"];

    function BoPhanSuDungCongCuController($scope, KendoGridOptionsFactory, $state) {
        var model = this;
        model.title = "BoPhanSuDungCongCu MANAGEMENT";
        ...
        model.mainGridOptions = new KendoGridOptionsFactory({
            dataSource: {
                ...
                transport: {
                    read: {
                        url: "http://localhost/accapi/bophansudungcongcus/list",
                        type: "POST",
