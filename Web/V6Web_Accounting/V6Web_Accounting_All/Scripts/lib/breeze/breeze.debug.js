  ...
  function parseCsdlEntityType(csdlEntityType, schema, metadataStore) {
      var shortName = csdlEntityType.name,
          ...
          entityObj = {
              ...
              namespace: getNamespaceFor(shortName, schema),
              isAbstract: (csdlEntityType.abstract && csdlEntityType.abstract === 'true')
          },
          entityType;
          
      switch (csdlEntityType.autoGeneratedKeyType) {
          case 'Identity':
              entityObj.autoGeneratedKeyType = AutoGeneratedKeyType.Identity;
              break;
          case 'KeyGenerator':
              entityObj.autoGeneratedKeyType = AutoGeneratedKeyType.KeyGenerator;
              break;
          default:
              entityObj.autoGeneratedKeyType = AutoGeneratedKeyType.None;
      }

    entityType = new EntityType(entityObj);
...
var DataProperty = (function () {
  ...
  **/
  var ctor = function DataProperty(config) {
      ...
      ac = ac.whereParam("isNullable").isBoolean();
      ac = ac.isOptional();
      ac = ac.withDefault(true);

      ac.whereParam("isScalar").isOptional().withDefault(true)// will be false for some NoSQL databases.
