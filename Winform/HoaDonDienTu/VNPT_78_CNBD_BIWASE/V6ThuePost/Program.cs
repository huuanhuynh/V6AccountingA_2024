...
namespace V6ThuePost
{
    static class Program
    {
            ...
            if (args != null && args.Length > 0)
            {
                ...
                try
                {
                    ...
                    else if (mode.StartsWith("M") || mode == "")
                    {
                        ...
                        else if (mode.StartsWith("MB"))
                        {
                            #region ==== mode M M1:tạo mới gửi excel có sẵn  M2:tạo mới+xuất excel rồi gửi.

                            if (mode == "MB9") // phát hành hóa đơn nháp
                            {
                                result = _vnptWS.PublishInvFkey_BIWASE(fkeyA, pattern, seri, out v6return);
                            }
                            else
                            {
                                result = _vnptWS.ImportAndPublishInv_BIWASE(xml, pattern, seri, out v6return);
                    ...
                    else if (mode.StartsWith("G"))
                    {
                        ...
                        if (mode.StartsWith("GB"))
                        {
                            if (mode == "GB1") // Gạch nợ theo fkey
                            {
                                File.Create(flagFileName1).Close();
                                result = _vnptWS.ConfirmPaymentFkeyPattern_BIWASE(arg2, pattern, seri, out v6return);
                            }
                            else if (mode == "GB2") // Gạch nợ hóa đơn theo lstInvToken(01GTKT2/001;AA/13E;10)
                            {
                                File.Create(flagFileName1).Close();
                                result = _vnptWS.ConfirmPaymentFkeyPattern_BIWASE(arg2, pattern, seri, out v6return);
                            }
                            else if (mode == "GB3") // Hủy gạch nợ theo fkey
                            {
                                File.Create(flagFileName1).Close();
                                result = _vnptWS.UnconfirmPaymentFkeyPattern_BIWASE(arg2, pattern, seri, out v6return);
                            }
                        }
                        else
                        {
        ...
        private static string FixReadMoney(DataRow row, string ReadFormatField, string old_read)
        {
            ...
            string format = null;
            if (ReadFormatField != null && row.Table.Columns.Contains(ReadFormatField)) format = (row[ReadFormatField] + "").Trim();
