...
namespace V6ThuePost
{
    static class Program
    {
        ...
        #region ===== VAR =====
        public static CYBER_WS _CYBER_WS = null;
        /// <summary>
        /// Link host
        /// </summary>
        ...
        static void Main(string[] args)
        {
            ...
            if (args != null && args.Length > 0)
            {
                ...
                try
                {
                    ...
                    _CYBER_WS = new CYBER_WS(baseUrl, username, password, _codetax);

                    if (mode.ToUpper() == "MTEST")
                    {
                        ReadData_CYBER(arg2, "M"); // đọc để lấy tên flag.
                        jsonBody = "";
                        File.Create(flagFileName1).Close();
                        result = _CYBER_WS.POST_CREATE_INVOICE(jsonBody, _SIGNMODE == "HSM", out v6return);
                    ...
                    else if (mode.ToUpper() == "MTEST_JSON")
                    {
                        ...
                        jsonBody = "";
                        result = _CYBER_WS.POST_CREATE_INVOICE(jsonBody, _version == "V45I", out v6return);
                    ...
                    else if (mode.StartsWith("M") && mode.EndsWith("_JSON"))
                    {
                        ...
                        {
                            result = _CYBER_WS.POST_CREATE_INVOICE(jsonBody, false, out v6return);
                        }
                        else
                        {
                            result = _CYBER_WS.POST_CREATE_INVOICE(jsonBody, _version == "V45I", out v6return);
                            ...
                            && string.IsNullOrEmpty(v6return.SO_HD))
                        {
                            ...
                            _CYBER_WS.SearchInvoiceByTransactionUuid(_codetax, _ma_hoadon_or_fkey, out v6return);
                    ...
                    else if (mode.StartsWith("M"))
                    {
                        ...
                        if (mode == "M0") // DRAF
                        {
                            jsonBody = ReadData_CYBER(dbfFile, "M");
                            File.Create(flagFileName1).Close();
                            result = _CYBER_WS.POST_CREATE_INVOICE(jsonBody, false, out v6return);
                        }
                        else if (mode == "M9")
                        {
                            result = _CYBER_WS.SIGN_HSM("MAGIAODICH", "MAHOADON", out v6return);
                        }
                        ...
                        else // Ký số client. /InvoiceAPI/InvoiceWS/createInvoiceUsbTokenGetHash/{supplierTaxCode}
                        {
                            ...
                            result = _CYBER_WS.CreateInvoiceUsbTokenGetHash_Sign(jsonBody, templateCode, _SERIAL_CERT, out v6return);
                        }
                    ...
                    {
                        jsonBody = ReadData_CYBER(dbfFile, "S");
                        File.Create(flagFileName1).Close();
                        result = _CYBER_WS.POST_EDIT(jsonBody, _SIGNMODE == "HSM", out v6return);
                    }
                    else if (mode.StartsWith("T") && mode.EndsWith("_JSON"))
                    {
                        // Lưu ý dữ liệu mode Replace. (T)
                        jsonBody = ReadText(arg2);
                        result = _CYBER_WS.POST_REPLACE(jsonBody, _SIGNMODE == "HSM", out v6return);
                    }
                    else if (mode == "T")
                    {
                        jsonBody = ReadData_CYBER(dbfFile, "T");
                        File.Create(flagFileName1).Close();
                        result = _CYBER_WS.POST_REPLACE(jsonBody, _SIGNMODE == "HSM", out v6return);
                    }
                    else if (mode == "GET_INVOICE" || mode == "GET_INVOICE_JSON")
                    {
                        ...
                        MakeFlagNames(uid);
                        File.Create(flagFileName1).Close();
                        result = _CYBER_WS.SearchInvoiceByTransactionUuid(_codetax, uid, out v6return);
                    ...
                    else if (mode == "H" || mode == "H_JSON")
                    {
                        ...
                        File.Create(flagFileName1).Close();
                        result = _CYBER_WS.HUY_HOA_DON(stt_rec, out v6return);
                    }
                    else if (mode.StartsWith("P"))
                    {
                        ...
                        if (mode.StartsWith("P2") || mode.StartsWith("PP2"))
                        {
                            MakeFlagNames(uid);
                            //string strIssueDate = V6JsonConverter.ObjectToJson(ngay_ct, "VIETTEL");
                            string info = _CYBER_WS.SearchInvoiceByTransactionUuid(_codetax, uid, out v6return);
                            CYBER_SearchInvoiceResponseV2 infoObj =  v6return.RESULT_OBJECT as CYBER_SearchInvoiceResponseV2;
                            if (infoObj != null)
                            {
                                string strIssueDate = "infoObj.result[0].issueDate";
                                try
                                {
                                    result = _CYBER_WS.DownloadInvoicePDFexchange(_codetax, soseri_soct, strIssueDate, V6SoftLocalAppData_Directory, out v6return);
                                }
                        ...
                        else
                        {
                            try
                            {
                                result = _CYBER_WS.TAI_HOA_DON_PDF(_codetax, soseri_soct, uid, V6SoftLocalAppData_Directory, out v6return);
                            }
                            catch (Exception download_ex)
                            {
                                result = download_ex.Message;
        ...
        /// <returns></returns>
        public static string ReadData_CYBER(string dbfFile, string mode)
        {
            string result = "";
            //try
            {
        ...
        private static object GetValue(DataRow row, ConfigLine config)
        {
            ...
            {
                configDATATYPE = (config.DataType ?? "");
            }
        ...
        private static CYBER_PostObject postObject;
