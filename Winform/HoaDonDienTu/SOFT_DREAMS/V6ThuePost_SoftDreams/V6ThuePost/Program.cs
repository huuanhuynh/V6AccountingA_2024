...
namespace V6ThuePost
{
    static class Program
    {
        ...
        /// <summary>
        /// File dbf, trường hợp không có file data thì là fkey để download...
        /// </summary>
        private static string arg2 = "";
        /// <summary>
        /// File upload mode M1, data mode M2, old_fkey mode S.
        /// </summary>
        private static string arg3 = "";
        /// <summary>
        /// File upload mode S, [save file pdf mode E]
        /// </summary>
            ...
            if (args != null && args.Length > 0)
            {
                ...
                try
                {
                    ...
                    if (mode.ToLower() == "DownloadInvPDFFkey".ToLower())
                    {
                        ...
                        MakeFlagNames(fkeyA);
                        V6Return v6return;
                        string invXml = _softDreams_ws.GetInvoicePdf(fkeyA, 0, pattern, seri, Application.StartupPath, out v6return);
                        //string so_hoa_don = GetSoHoaDon_xml(invXml);
                        WriteFlag(flagFileName4, v6return.PATH);
                    ...
                    else if (mode.ToLower() == "DownloadInvPDFFkeyNoPay".ToLower())
                    {
                        ...
                        fkeyA = arg2;
                        MakeFlagNames(fkeyA);
                        string invXml = _softDreams_ws.DownloadInvPDFFkeyNoPay(fkeyA);
                    ...
                    //    result = _softDreams_ws.ImportAndPublishInv(new Invoices(), );
                    ...
                    {
                        V6Return v6return;
                        var xml = _softDreams_ws.GetInvoicePdf(fkeyA, 0, pattern, seri, Application.StartupPath, out v6return);
                        result += v6return.RESULT_STRING;
                    }
                    ...
                    else if (mode.StartsWith("M") || mode == "")
                    {
                        ...
                        var invoices = ReadDataXml(arg2) as Invoices;
                        File.Create(flagFileName1).Close();
                        V6Return v6return;
                        ...
                        if (arg3.Length > 0 && result.StartsWith("OK"))
                        {
                            if (mode.EndsWith("1")) //Gửi file excel có sẵn
                            {
                                if (File.Exists(arg3))
                                {
                                    result += _softDreams_ws.UploadInvAttachmentFkey(fkeyA, arg3);
                                }
                                else
                                {
                            ...
                            else if (mode.EndsWith("2")) // Tự xuất excel rồi gửi.
                            {
                                ...
                                {
                                    result += _softDreams_ws.UploadInvAttachmentFkey(fkeyA, export_file);
                                }
                            }
                    ...
                    else
                    {
                        ...
                        if (mode == "S")
                        {
                            var inv = ReadDataXmlS(dbfFile: arg2) as AdjustInv;
                            File.Create(flagFileName1).Close();
                            result = _softDreams_ws.AdjustInvoice(inv, arg3, pattern, seri, false, _signmode, out v6return);
                            if (arg4.Length > 0 && result.StartsWith("OK"))
                            {
                                if (File.Exists(arg4))
                                {
                                    result += _softDreams_ws.UploadInvAttachmentFkey(fkeyA, arg4);
                                }
                                else
                                {
                        ...
                        {
                            var inv = ReadDataXmlT(arg2) as ReplaceInv;
                            File.Create(flagFileName1).Close();
                            result = _softDreams_ws.ReplaceInvoice(inv, arg3, pattern, seri, false, _signmode, out v6return);
                        }
                        else if (mode.StartsWith("G"))
                        {
                            ...
                            {
                                File.Create(flagFileName1).Close();
                                result = _softDreams_ws.ConfirmPaymentFkey(arg2, out v6return);
                            }
                            else if (mode == "G2") // Gạch nợ hóa đơn theo lstInvToken(01GTKT2/001;AA/13E;10)
                            {
                                File.Create(flagFileName1).Close();
                                result = _softDreams_ws.ConfirmPayment(arg2, out v6return);
                            }
                            else if (mode == "G3") // Hủy gạch nợ theo fkey
                            {
                                File.Create(flagFileName1).Close();
                                result = _softDreams_ws.UnconfirmPaymentFkey(arg2, out v6return);
                            }
                        }
                        else if (mode == "H")
                        {
                            MakeFlagNames(arg2);
                            File.Create(flagFileName1).Close();
                            result = _softDreams_ws.CancelInvoice(arg2, pattern, seri);
                        }
                        else if (mode.StartsWith("U"))//U1,U2
                        {
                            if (mode == "U")        // upload file có sẵn, fkey truyền vào
                            {
                                ...
                                string file = arg3;
                                MakeFlagNames(fkey);
                                result = _softDreams_ws.UploadInvAttachmentFkey(fkey, file);
                            }
                            else if (mode == "U1") // upload file có sẵn, fkey tự đọc từ data
                            {
                                ...
                                string fkey = fkeyA;
                                MakeFlagNames(fkey);
                                result = _softDreams_ws.UploadInvAttachmentFkey(fkey, fkey + ".xls");
                            }
        ...
        public static Invoices ReadDataXml(string dbfFile)
        {
        ...
        public static AdjustInv ReadDataXmlS(string dbfFile)
        {
            AdjustInv inv;
            //try
            {
        ...
        /// <param name="dbfFile"></param>
        /// <returns></returns>
        public static ReplaceInv ReadDataXmlT(string dbfFile)
        {
            ReplaceInv inv;
            //try
            {
        ...
        private static object GetValue(DataRow row, ConfigLine config)
        {
            ...
            if (!string.IsNullOrEmpty(configDATATYPE))
            {
                switch (configDATATYPE)
                {
                    ...
                    case "UPPER1": // Chỉ dùng ở exe gọi bằng Foxpro.
                        string s = fieldValue + "";
                        if (s.Length > 0)
                        {
        ...
        public static string UpdateCus(string xml)
        {
            ...
            try
            {
                result = 0;// = new PublishService(link_Publish).UpdateCus(xml, username, password, 0);
                message += result;
