...
namespace V6Tools
{
    public class ChuyenMaTiengViet
    {
        #region ==== HOA thường ====
        ...
        /// <summary>
        /// Viết hoa chữ đầu tiên trong câu, giữ nguyên phần còn lại.
        /// </summary>
        /// <param name="s"></param>
        /// <returns></returns>
        public static string HoaDauCau(string s)
        {

            if (string.IsNullOrEmpty(s)) return s;
            string rs = "";
            string space = ".:;[]{}?!()";
            char prc = ' ';// lưu ký tự trước đó.
            bool new_block = true;
            foreach (char c in s)
            {
                if (new_block)
        ...
        /// <summary>
        /// Dù trong đoạn có nhiều câu thì cũng chỉ Upper ký tự đầu tiên.
        /// </summary>
        /// <param name="s"></param>
        /// <returns></returns>
        public static string HoaDauDoan(string s)
        {
        ...
        //                                                                                         v  có ký tự giống dấu - sau chữ ô
        static readonly string strTCVN =    "¸µ¶·¹¨¾»¼½Æ©ÊÇÈÉËÐÌÎÏÑªÕÒÓÔÖÝ×ØÜÞãßáâä«èåæçé¬íêëìîóïñòô­øõö÷ùýúûüþ®¸µ¶·¹¡¾»¼½Æ¢ÊÇÈÉËÐÌÎÏÑ£ÕÒÓÔÖÝ×ØÜÞãßáâä¤èåæçé¥íêëìîóïñòô¦øõö÷ùýúûüþ§§";//2 ký tự sau ô kô hiển thị
        private static int[] TCVN = { 184, 181, 182, 183, 185, 168, 190, 187, 188, 189, 198, 169, 202, 199, 200, 201, 203, 208, 204, 206, 207, 209, 170, 213, 210, 211, 212, 214, 221, 215, 216, 220, 222, 227, 223, 225, 226, 228, 171, 232, 229, 230, 231, 233, 172, 237, 234, 235, 236, 238, 243, 239, 241, 242, 244, 173, 248, 245, 246, 247, 249, 253, 250, 251, 252, 254, 174, 184, 181, 182, 183, 185, 161, 190, 187, 188, 189, 198, 162, 202, 199, 200, 201, 203, 208, 204, 206, 207, 209, 163, 213, 210, 211, 212, 214, 221, 215, 216, 220, 222, 227, 223, 225, 226, 228, 164, 232, 229, 230, 231, 233, 165, 237, 234, 235, 236, 238, 243, 239, 241, 242, 244, 166, 248, 245, 246, 247, 249, 253, 250, 251, 252, 254, 167, 167 };


        static readonly string strUNICODE = "áàảãạăắằẳẵặâấầẩẫậéèẻẽẹêếềểễệíìỉĩịóòỏõọôốồổỗộơớờởỡợúùủũụưứừửữựýỳỷỹỵđÁÀẢÃẠĂẮẰẲẴẶÂẤẦẨẪẬÉÈẺẼẸÊẾỀỂỄỆÍÌỈĨỊÓÒỎÕỌÔỐỒỔỖỘƠỚỜỞỠỢÚÙỦŨỤƯỨỪỬỮỰÝỲỶỸỴĐÐ";//Đ272, Ð208 
        static int[] UNICODE = {
        ...
        private static readonly string[] strVNIarray = "aù,aø,aû,aõ,aï,aê,aé,aè,aú,aü,aë,aâ,aá,aà,aå,aã,aä,eù,eø,eû,eõ,eï,eâ,eá,eà,eå,eã,eä,í,ì,æ,ó,ò,où,oø,oû,oõ,oï,oâ,oá,oà,oå,oã,oä,ô,ôù,ôø,ôû,ôõ,ôï,uù,uø,uû,uõ,uï,ö,öù,öø,öû,öõ,öï,yù,yø,yû,yõ,î,ñ,AÙ,AØ,AÛ,AÕ,AÏ,AÊ,AÉ,AÈ,AÚ,AÜ,AË,AÂ,AÁ,AÀ,AÅ,AÃ,AÄ,EÙ,EØ,EÛ,EÕ,EÏ,EÂ,EÁ,EÀ,EÅ,EÃ,EÄ,Í,Ì,Æ,Ó,Ò,OÙ,OØ,OÛ,OÕ,OÏ,OÂ,OÁ,OÀ,OÅ,OÃ,OÄ,Ô,ÔÙ,ÔØ,ÔÛ,ÔÕ,ÔÏ,UÙ,UØ,UÛ,UÕ,UÏ,Ö,ÖÙ,ÖØ,ÖÛ,ÖÕ,ÖÏ,YÙ,YØ,YÛ,YÕ,Î,Ñ,Ñ".Split(',');
