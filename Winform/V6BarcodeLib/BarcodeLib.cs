...
using System.IO;
using V6BarcodeLib.Symbologies;
...
namespace V6BarcodeLib
{
    ...
    public class BarcodeLib: IDisposable
    {
        ...
        private BARCODE_TYPE _encodedBarcodeType = BARCODE_TYPE.UNSPECIFIED;
        private Image _Encoded_Image = null;
        private Color _ForeColor = Color.Black;
        private Color _BackColor = Color.White;
        private float _Width = 37.29f;
        private float _Height = 18.93f;
        ...
        public BarcodeLib(string data, BARCODE_TYPE iType)
        {
            this.Raw_Data = data;
            this._encodedBarcodeType = iType;
        }
        #endregion
        ...
        /// </summary>
        public BARCODE_TYPE EncodedBarcodeType
        {
            set { _encodedBarcodeType = value; }
            get { return _encodedBarcodeType;  }
        }//EncodedType
        /// <summary>
        ...
        /// <summary>
        /// Gets or sets the width of the image to be drawn. (Default is 37.29 milimet)
        /// </summary>
        public float Width
        {
            get { return _Width; }
        ...
        /// <summary>
        /// Gets or sets the height of the image to be drawn. (Default is 18.93 milimet)
        /// </summary>
        public float Height
        {
            get { return _Height; }
        ...
        {
            get { return _fCodeTextFontSize; }
            set { _fCodeTextFontSize = value; }
        }
        /// <summary>
        ...
        /// <summary>
        /// Vẽ barcode với những tham số đã nhập vào trước.
        /// </summary>
        /// <param name="g"></param>
        /// <param name="pt"></param>
        public void DrawBarcode(Graphics g, Point pt)
        {
            DrawBarcode(g, pt, Raw_Data, ProductCode, ProductName, ProductPrice);
        }

        /// <summary>
        /// Vẽ barcode vào graphics theo đơn vị milimet.
        /// </summary>
        /// <param name="g">Graphics</param>
        /// <param name="pt">Vị trí bắt đầu</param>
        /// <param name="value">Giá trị đầu vào barcode</param>
        /// <param name="productPrice"></param>
        /// <param name="btype">Loại barcode</param>
        ...
        public void DrawBarcode(Graphics g, Point pt, string value,
            ...
            BARCODE_TYPE btype = BARCODE_TYPE.UNSPECIFIED)
        {
            if (btype != BARCODE_TYPE.UNSPECIFIED)
            {
                EncodedBarcodeType = btype;
            }
            _sProductCode = productCode;
            _sProductName = productName;
            _decProductPrice = productPrice;
            Raw_Data = value;
            ...
            {
                throw new Exception(string.Format("({0}) Encode error: {1}", Raw_Data, ex.Message));
            }
            var encodedString = Encoded_Value;
            ...
            Font fontOfNameText = new Font("Arial", this.NameTextFontSize * this.Scale,
                this.NameTextFontBold ? FontStyle.Bold : FontStyle.Regular);
            Font fontOfCodeText = new Font("Arial", this._fCodeTextFontSize * this.Scale,
                this.CodeTextFontBold ? FontStyle.Bold : FontStyle.Regular);
            Font fontOfPriceText = new Font("Arial", this.PriceTextFontSize * this.Scale,
                this.PriceTextFontBold ? FontStyle.Bold : FontStyle.Regular);


            string sTempUPC = "000000000" + encodedString + "000000000";
            ...
            float fTextHeight = g.MeasureString(sTempUPC, fontOfCodeText).Height;
            ...
            // Vẽ những đường mã vạch
            float lineWidth = width / (float)(encodedString.Length + 16);
            //xPosition = lineWidth*9;

            for (int i = 0; i < sTempUPC.Length; i++)
            {
                if (sTempUPC.Substring(i, 1) == "1")
                {
                    if (EncodedBarcodeType == BARCODE_TYPE.EAN13 && (i<=12 || (55<=i&&i<=57) || 101<=i))
                    {
                        //Vẽ dài đến hết phần mã.
                        g.FillRectangle(brush, xStart + xPosition, yStart, lineWidth,
                            height - nameHeight - fPriceTextHeight);
                    }
                    else
                    {
                        //Vẽ ngắn thông thường, chưa qua phần mã.
                        g.FillRectangle(brush, xStart + xPosition, yStart, lineWidth,
                            height - nameHeight - fPriceTextHeight - fTextHeight);
                    }
            ...
            if (EncodedBarcodeType == BARCODE_TYPE.EAN13)
            {
                //xPosition = xStart - g.MeasureString(this.CountryCode.Substring(0, 1), fontOfCodeText).Width;
                xPosition = xStart;
                yPosition = yStart + (height - nameHeight - fPriceTextHeight - fTextHeight);

                // Draw 1st digit of the country code. Số đầu tiên của mã quốc gia.
                g.DrawString(sData.Substring(0, 1), fontOfCodeText, brush, new PointF(xStart + xPosition, yPosition));

                xPosition += (g.MeasureString(sData.Substring(0, 1), fontOfCodeText).Width + 43*lineWidth) -
                             (g.MeasureString(sData.Substring(1, 6), fontOfCodeText).Width);

                // Draw MFG Number. Phần code số bên trái
                g.DrawString(sData.Substring(1, 6), fontOfCodeText, brush, new PointF(xStart + xPosition, yPosition));

                xPosition += g.MeasureString(sData.Substring(1, 6), fontOfCodeText).Width + (11*lineWidth);

                // Draw Product ID. Phần code số bên phải
                g.DrawString(sData.Substring(7), fontOfCodeText, brush, new PointF(xStart + xPosition, yPosition));
            }
            else
            {
                float dataTextWidth = g.MeasureString(sData, fontOfCodeText).Width;
                xPosition = (width-dataTextWidth)/2;//reset left for center text
                if (xPosition < 0) xPosition = 0;
                yPosition = yStart + (height - nameHeight - fPriceTextHeight - fTextHeight);
                g.DrawString(sData, fontOfCodeText, brush, new PointF(xStart + xPosition, yPosition));
            }
        ...
        public string Encode(BARCODE_TYPE iType, string StringToEncode, int Width, int Height)
        {
            this.Width = Width;
            this.Height = Height;
        ...
        /// <param name="Height">Height of the resulting barcode.(pixels)</param>
        /// <returns>Image representing the barcode.</returns>
        public string Encode(BARCODE_TYPE iType, string StringToEncode, Color ForeColor, Color BackColor, int Width, int Height)
        {
            this.Width = Width;
            this.Height = Height;
        ...
        /// <param name="BackColor">Background color</param>
        /// <returns>Image representing the barcode.</returns>
        public string Encode(BARCODE_TYPE iType, string StringToEncode, Color ForeColor, Color BackColor)
        {
        ...
        /// <param name="StringToEncode">Raw data to encode.</param>
        /// <returns>Image representing the barcode.</returns>
        public string Encode(BARCODE_TYPE iType, string StringToEncode)
        {
        ...
        /// </summary>
        /// <param name="iType">Type of encoding to use.</param>
        internal string Encode(BARCODE_TYPE iType)
        {
            _encodedBarcodeType = iType;
            return Encode();
        }//Encode()
        /// <summary>
        ...
        internal string Encode()
        {
            ...
            switch (this._encodedBarcodeType)
            {
                ...
                case BARCODE_TYPE.Modified_Plessey: //Encode_MSI();
                    ibarcode = new MSI(Raw_Data, _encodedBarcodeType);
                    break;
                ...
                case BARCODE_TYPE.ITF14:
                    {
                        ...
                        b = new Bitmap((int) (Width*3), (int) (Height*3));
                            ...
                            using (Pen pen = new Pen(ForeColor, iBarWidth))
                            {
                                ...
                                while (pos < Encoded_Value.Length)
                                {
                                    ...
                                    if (Encoded_Value[pos] == '1')
                                        g.DrawLine(pen, new Point((pos * iBarWidth) + shiftAdjustment + bearerwidth + iquietzone, 0), new Point((pos * iBarWidth) + shiftAdjustment + bearerwidth + iquietzone, (int)(Height * 3)));
                ...
                default:
                    {
                        ...
                        b = new Bitmap((int)(Width * 3), (int)(Height * 3));
                        int iBarWidth = (int) (Width*3)/Encoded_Value.Length;
                        int shiftAdjustment = 0;
                        int iBarWidthModifier = 1;

                        if (this._encodedBarcodeType == BARCODE_TYPE.PostNet)
                            iBarWidthModifier = 2;

                        //set alignment
                        switch (Alignment)
                        {
                            case AlignmentPositions.CENTER: shiftAdjustment = ((int)(Width*3) % Encoded_Value.Length) / 2;
                                break;
                            case AlignmentPositions.LEFT: shiftAdjustment = 0;
                                break;
                            case AlignmentPositions.RIGHT: shiftAdjustment = ((int)(Width * 3) % Encoded_Value.Length);
                                break;
                            default: shiftAdjustment = ((int)(Width * 3) % Encoded_Value.Length) / 2;
                                break;
                        }//switch
                                    ...
                                    while (pos < Encoded_Value.Length)
                                    {
                                        if (this._encodedBarcodeType == BARCODE_TYPE.PostNet)
                                        {
                                            ...
                                            if (Encoded_Value[pos] == '0')
                                                g.DrawLine(pen, new Point(pos * iBarWidth + shiftAdjustment + halfBarWidth, (int)(Height * 3)), new Point(pos * iBarWidth + shiftAdjustment + halfBarWidth, (int)(Height * 3) / 2));
                                            else
                                                g.DrawLine(pen, new Point(pos * iBarWidth + shiftAdjustment + halfBarWidth, (int)(Height * 3)), new Point(pos * iBarWidth + shiftAdjustment + halfBarWidth, 0));
                                        }//if
                                        else
                                        {
                                            if (Encoded_Value[pos] == '1')
                                                g.DrawLine(pen, new Point(pos * iBarWidth + shiftAdjustment + halfBarWidth, 0), new Point(pos * iBarWidth + shiftAdjustment + halfBarWidth, (int)(Height * 3)));
                                        }
                                        pos++;
                                    }//while
                                }//using
        ...
        /// <returns>Image representing the barcode.</returns>
        public static string DoEncode(BARCODE_TYPE iType, string Data)
        {
        ...
        /// <returns>Image representing the barcode.</returns>
        public static string DoEncode(BARCODE_TYPE iType, string Data, ref string XML)
        {
            using (BarcodeLib b = new BarcodeLib())
            {
        ...
        /// <returns>Image representing the barcode.</returns>
        public static string DoEncode(BARCODE_TYPE iType, string Data, bool IncludeLabel)
        {
            using (BarcodeLib b = new BarcodeLib())
            {
        ...
        /// <param name="Height">Height of the resulting barcode.(pixels)</param>
        /// <returns>Image representing the barcode.</returns>
        public static string DoEncode(BARCODE_TYPE iType, string Data, bool IncludeLabel, int Width, int Height)
        {
            using (BarcodeLib b = new BarcodeLib())
            {
                b.IncludeLabel = IncludeLabel;
        ...
        /// <param name="BackColor">Background color</param>
        /// <returns>Image representing the barcode.</returns>
        public static string DoEncode(BARCODE_TYPE iType, string Data, bool IncludeLabel, Color DrawColor, Color BackColor)
        {
            using (BarcodeLib b = new BarcodeLib())
            {
                b.IncludeLabel = IncludeLabel;
        ...
        /// <param name="Height">Height of the resulting barcode.(pixels)</param>
        /// <returns>Image representing the barcode.</returns>
        public static string DoEncode(BARCODE_TYPE iType, string Data, bool IncludeLabel, Color DrawColor, Color BackColor, int Width, int Height)
        {
            using (BarcodeLib b = new BarcodeLib())
            {
                b.IncludeLabel = IncludeLabel;
        ...
        /// <param name="XML">XML representation of the data and the image of the barcode.</param>
        /// <returns>Image representing the barcode.</returns>
        public static string DoEncode(BARCODE_TYPE iType, string Data, bool IncludeLabel, Color DrawColor, Color BackColor, int Width, int Height, ref string XML)
        {
            using (BarcodeLib b = new BarcodeLib())
            {
                b.IncludeLabel = IncludeLabel;
