    ...
    public class V6Invoice72 : V6InvoiceBase
    {
            ...
            List<IDictionary<string, object>> adList3)
        {
            ...
            else // insert không đủ dòng.
            {
                ...
                Logger.WriteToLog(string.Format("{0} Invoice72.InsertInvoice else.{1} {2}", V6Login.ClientName, stt_rec, V6Message));
            }
            return false;
        }
            ...
            List<IDictionary<string, object>> adList3)
        {
            ...
            if (insert_success && j == adList.Count && j2 == adList2.Count && j3 == adList3.Count)
            {
                ...
                try
                {
                    ...
                    var result = SqlHelper.ExecuteNonQuery(DatabaseConfig.ConnectionString2_TH, CommandType.StoredProcedure, "VPA_POB_POST_MAIN", plist);
            ...
            IDictionary<string, object> keys)
        {
            ...
            if (insert_success && j == adList.Count && j2 == adList2.Count && j3 == adList3.Count)
            {
                ...
                try
                {
                    ...
                    SqlHelper.ExecuteNonQuery(DatabaseConfig.ConnectionString2_TH, CommandType.StoredProcedure, "VPA_POB_POST_MAIN", plist);
                    return true;
                }
                catch (Exception ex)
                {
        ...
        public override DataTable LoadAD(string sttRec)
        {
            ...
            string sql = "SELECT c.*,d.Ten_vt AS Ten_vt, e.Ton13 " + ADSELECTMORE + " FROM " + AD_TableName
                + " c LEFT JOIN Alvt d ON c.Ma_vt= d.Ma_vt LEFT JOIN abvt13 e ON c.Ma_vt = e.Ma_vt AND c.Ma_kho_i = e.Ma_kho ";
            sql += ADJOINMORE;
