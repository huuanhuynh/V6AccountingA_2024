    ...
    public class V6Invoice82 : V6InvoiceBase
    {
            ...
            List<IDictionary<string, object>> adList3)
        {
            ...
            else // insert không đủ dòng.
            {
                ...
                Logger.WriteToLog(string.Format("{0} Invoice82.InsertInvoice else.{1} {2}", V6Login.ClientName, stt_rec, V6Message));
            }
            return false;
        }
            ...
            List<IDictionary<string, object>> adList3)
        {
            ...
            if (insert_success && j == adList.Count && j3 == adList3.Count)
            {
                ...
                try
                {
                    var result = SqlHelper.ExecuteNonQuery(DatabaseConfig.ConnectionString2_TH, CommandType.StoredProcedure, "VPA_SOB_POST_MAIN", pList);
                    V6Message = string.Format("Success, ({0} affected).", result);
                    return true;
                }
                catch (Exception ex)
            ...
            IDictionary<string, object> keys)
        {
            ...
            int j = 0, j3 = 0;
            var amSql = SqlGenerator.GenUpdateAMSql(V6Login.UserId, AMStruct.TableName, AMStruct, amData, keys);
            SqlTransaction TRANSACTION = SqlConnect.CreateSqlTransaction2_TH("Update82");
