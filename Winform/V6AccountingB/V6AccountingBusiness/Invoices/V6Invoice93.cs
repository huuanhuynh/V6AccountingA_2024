...
namespace V6AccountingBusiness.Invoices
{
    ...
    /// SOR: Báo giá.
    /// </summary>
    public class V6Invoice93 : V6InvoiceBase
    {
        public V6Invoice93() : base("SOR") { }
        ...
        public override bool InsertInvoice(IDictionary<string, object> amData, List<IDictionary<string, object>> adList)
        {
            ...
            if (insert_success && j == adList.Count)
            {
                ...
                try
                {
                    ...
                    V6BusinessHelper.ExecuteProcedureNoneQuery("VPA_SOR_POST_MAIN", pList);
            ...
            else // insert không đủ dòng.
            {
                ...
                Logger.WriteToLog(string.Format("{0} Invoice93.InsertInvoice else.{1} {2}", V6Login.ClientName, stt_rec, V6Message));
            }
            return false;
        }
        ...
        public override bool InsertInvoice2_TH(IDictionary<string, object> amData, List<IDictionary<string, object>> adList)
        {
            ...
            if (insert_success && j == adList.Count)
            {
                ...
                try
                {
                    ...
                    var result = SqlHelper.ExecuteNonQuery(DatabaseConfig.ConnectionString2_TH, CommandType.StoredProcedure, "VPA_SOR_POST_MAIN", pList);
            ...
            IDictionary<string, object> keys)
        {
            ...
            SqlTransaction TRANSACTION = SqlConnect.CreateSqlTransaction("Invoice93Update");
            ...
            IDictionary<string, object> keys)
        {
            ...
            var j = 0;
            var amSql = SqlGenerator.GenUpdateAMSql(V6Login.UserId, AM_TableName, AMStruct, amData, keys);
            SqlTransaction TRANSACTION = SqlConnect.CreateSqlTransaction2_TH("Invoice93Update");
            ...
            if (insert_success && j == adList.Count)
            {
                ...
                try
                {
                    ...
                    SqlHelper.ExecuteNonQuery(DatabaseConfig.ConnectionString2_TH, CommandType.StoredProcedure, "VPA_SOR_POST_MAIN", pList);
        ...
        public DataTable SearchBaoGia(string where0Ngay, string where1AM, string where2AD, string where3NhVt, string where4Dvcs, string where4Dvcs_2, out string loai_ct_chon)
        {
            if (where0Ngay.Length > 0) where0Ngay = "And " + where0Ngay;
            if (where1AM.Length > 0) where1AM = "And " + where1AM;
            ...
            if (where2AD.Length > 0 || where3NhVt.Length > 0 || where4Dvcs.Length > 0)
            {
                ...
                whereAD_Nhvt_Dvcs = string.Format("\n Where d.Stt_rec in (SELECT Stt_rec FROM [" + AD_TableName + "] WHERE Ma_ct = 'SOR' {0} {2}"
                                     + (where3NhVt.Length == 0 ? "{3}" : "\n	And Ma_vt IN (SELECT Ma_vt FROM Alvt WHERE 1 = 1 {3})")
                                     + "\n		{4})"
            ...
            var sql = string.Format("Select ' ' Tag,  v.ten_vt,v.tk_tl , d.*, d.STT_REC AS STT_REC_PX, d.STT_REC0 AS STT_REC0PX "
                ...
                + "\n  JOIN (SELECT Stt_rec FROM ["+AM_TableName+"] WHERE Ma_ct = 'SOR'" + "\n {0} {1}) AS m ON d.Stt_rec = m.Stt_rec"
                + "\n {2}"
        ...
        public override bool DeleteInvoice(string sttrec)
        {
            try
            {
                ...
                V6BusinessHelper.ExecuteProcedureNoneQuery("VPA_SOR_DELETE_MAIN", plist);
                return true;
            }
            catch (Exception ex)
            {
