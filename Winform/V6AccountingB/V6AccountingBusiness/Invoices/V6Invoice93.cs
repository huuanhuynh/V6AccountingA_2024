...
namespace V6AccountingBusiness.Invoices
{
    ...
    public class V6Invoice93 : V6InvoiceBase
    {
        ...
        public override string Name { get { return "Báo giá"; } }
        ...
        public DataTable SearchAM(SortedDictionary<string,object> keys, SortedDictionary<string,object> keys_adv)
            //where0Ngay, string where1AM, string where2AD, string where3NhVt, string where4Dvcs)
        {
            //string template =
            ...
            //    p2Template = string.Format(p2Template, w0, "", where2AD, where3NhVt, where4Dvcs);
            //}
            //else
            //{
            //    p2Template = "";
        ...
        public DataTable SearchBaoGia(string where0Ngay, string where1AM, string where2AD, string where3NhVt, string where4Dvcs)
        {
            ...
            if (where2AD.Length > 0 || where3NhVt.Length > 0 || where4Dvcs.Length > 0)
            {
                ...
                whereAD_Nhvt_Dvcs = string.Format("\n Where d.Stt_rec in (SELECT Stt_rec FROM [" + AD + "] WHERE Ma_ct = 'SOR' {0} {2}"
                                     + (where3NhVt.Length == 0 ? "{3}" : "\n	And Ma_vt IN (SELECT Ma_vt FROM Alvt WHERE 1 = 1 {3})")
                                     + "\n		{4})"
            ...
            var sql = string.Format("Select ' ' Tag,  v.ten_vt,v.tk_tl , d.*, d.STT_REC AS STT_REC_PX, d.STT_REC0 AS STT_REC0PX "
                + "\nFROM ["+AD+"] d "//" LEFT JOIN Alkh b ON d.Ma_kh=b.Ma_kh "
                + "\n LEFT JOIN Alvt v ON v.Ma_vt = d.Ma_vt "
                + "\n  JOIN (SELECT Stt_rec FROM ["+AM+"] WHERE Ma_ct = 'SOR'" + "\n {0} {1}) AS m ON d.Stt_rec = m.Stt_rec"
                + "\n {2}"
