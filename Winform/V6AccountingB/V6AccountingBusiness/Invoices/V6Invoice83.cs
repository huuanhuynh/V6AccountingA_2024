    ...
    public class V6Invoice83 : V6InvoiceBase
    {
            ...
            bool post)
        {
            ...
            else // insert không đủ dòng.
            {
                ...
                Logger.WriteToLog(string.Format("{0} Invoice83.InsertInvoice else.{1} {2}", V6Login.ClientName, stt_rec, V6Message));
            }
            return false;
        }
            ...
            bool post)
        {
            ...
            if (insert_success && j == adList.Count && j3 == adList3.Count)
            {
                ...
                if (!post)
                {
                    ...
                    try
                    {
                        var result = SqlHelper.ExecuteNonQuery(DatabaseConfig.ConnectionString2_TH, CommandType.StoredProcedure, "VPA_SOC_POST_MAIN_AMAD", pList1);
                        V6Message = string.Format("Success, ({0} affected).", result);
                        return true;
                    }
                    catch (Exception ex)
            ...
            IDictionary<string, object> keys, bool post)
        {
            ...
            if (insert_success && j == adList.Count && j3 == adList3.Count)
            {
                ...
                if (!post)
                {
                    ...
                    try
                    {
                        var result = SqlConnect.ExecuteNonQuery(CommandType.StoredProcedure, "VPA_SOC_POST_MAIN_AMAD",
                            pList1);
                        V6Message = string.Format("Success, ({0} affected).", result);
                        return true;
                    }
        ...
        public override bool DeleteInvoice(string sttrec)
        {
                ...
                SqlParameter[] plist =
            {
                ...
                SqlConnect.ExecuteNonQuery(CommandType.StoredProcedure, "VPA_SOC_DELETE_MAIN", plist);
                return true;
            }
            catch (Exception ex)
            {
