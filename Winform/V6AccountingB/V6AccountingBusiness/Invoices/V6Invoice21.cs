...
namespace V6AccountingBusiness.Invoices
{
    ...
    /// AR1: Hóa đơn dịch vụ
    /// </summary>
    public class V6Invoice21 : V6InvoiceBase
    {
        public V6Invoice21() : base("AR1") { }
        ...
        public override bool InsertInvoice(IDictionary<string, object> amData, List<IDictionary<string, object>> adList, List<IDictionary<string, object>> adList3)
        {
            ...
            if (insert_success && j == adList.Count && j3 == adList3.Count)
            {
                ...
                try
                {
                    ...
                    V6BusinessHelper.ExecuteProcedureNoneQuery("VPA_AR1_POST_MAIN", pList);
            ...
            else // insert không đủ dòng.
            {
                ...
                Logger.WriteToLog(string.Format("{0} Invoice21.InsertInvoice else.{1} {2}", V6Login.ClientName, stt_rec, V6Message));
            }
            return false;
        }

        public override bool InsertInvoice2_TH(IDictionary<string, object> amData, List<IDictionary<string, object>> adList, List<IDictionary<string, object>> adList3)
        {
            object stt_rec = amData["STT_REC"];
            bool insert_success = false;
            int j = 0, j3 = 0;
            ...
            if (insert_success && j == adList.Count && j3 == adList3.Count)
            {
                ...
                try
                {
                    ...
                    var result = SqlHelper.ExecuteNonQuery(DatabaseConfig.ConnectionString2_TH, CommandType.StoredProcedure, "VPA_AR1_POST_MAIN", pList);
            ...
            IDictionary<string, object> keys)
        {
            ...
            if (insert_success && j == adList.Count && j3 == adList3.Count)
            {
                ...
                try
                {
                    ...
                    SqlHelper.ExecuteNonQuery(DatabaseConfig.ConnectionString2_TH, CommandType.StoredProcedure, "VPA_AR1_POST_MAIN", pList);
        ...
        public override DataTable LoadAD(string sttRec)
        {
            string sql = "SELECT c.*,d.Ten_tk AS Ten_tk " + ADSELECTMORE + " FROM [" + AD_TableName
                + "] c LEFT JOIN Altk d ON c.tk_dt= d.tk ";
            sql += ADJOINMORE;
        ...
        public override bool DeleteInvoice(string sttrec)
        {
            try
            {
                ...
                V6BusinessHelper.ExecuteProcedureNoneQuery("VPA_AR1_DELETE_MAIN", plist);
                return true;
            }
            catch (Exception ex)
            {
