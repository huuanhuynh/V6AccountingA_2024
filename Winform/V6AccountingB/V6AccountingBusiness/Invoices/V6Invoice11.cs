...
namespace V6AccountingBusiness.Invoices
{
    ...
    public class V6Invoice11 : V6InvoiceBase
    {
        ...
        public override DataTable GetSoDu0TK_TienLe(string sttRec, string madvcs, DateTime ngay_ct, decimal soTienLe, string type, string tk_du, string filterString)
        {
            SqlParameter[] plist =
            {
                ...
                new SqlParameter("@SoTienLe", soTienLe),
                new SqlParameter("@Type", type),
                new SqlParameter("@TK_DU", tk_du),
                new SqlParameter("@advance", filterString)
            };
            var result = V6BusinessHelper.ExecuteProcedure("AGLCTGL1_CUSTS_InitSd_TK_TIENLE", plist).Tables[0];
            Alct0 = result;
            return Alct0;
        }
            ...
            List<IDictionary<string, object>> adList, List<IDictionary<string, object>> adList2)
        {
            ...
            else // insert không đủ dòng.
            {
                ...
                Logger.WriteToLog(string.Format("{0} Invoice11.InsertInvoice else.{1} {2}", V6Login.ClientName, stt_rec, V6Message));
            }
            return false;
        }
            ...
            List<IDictionary<string, object>> adList, List<IDictionary<string, object>> adList2)
        {
            ...
            if (insert_success && j == adList.Count && j2 == adList2.Count)
            {
                ...
                try
                {
                    ...
                    var result = SqlHelper.ExecuteNonQuery(DatabaseConfig.ConnectionString2_TH, CommandType.StoredProcedure, "VPA_GL1_POST_MAIN", pList);
        ...
        {
            string template =
                "Select top " + (_alctConfig.M_SL_CT0 == "0" ? "5" : _alctConfig.M_SL_CT0) + " a.*,f.Ten_nvien AS Ten_nvien "
                + AMSELECTMORE
                + "\nFROM "+AM_TableName+" a  LEFT JOIN alnvien f ON a.Ma_nvien=f.Ma_nvien "
                + AMJOINMORE
                + "\n JOIN "
