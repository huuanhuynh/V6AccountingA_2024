    ...
    public class V6Invoice97INT : V6InvoiceBase
    {
            ...
            List<IDictionary<string, object>> adList, List<IDictionary<string, object>> adList2)
        {
            ...
            else // insert không đủ dòng.
            {
                ...
                Logger.WriteToLog(string.Format("{0} Invoice97INT.InsertInvoice else.{1} {2}", V6Login.ClientName, stt_rec, V6Message));
            }
            return false;
        }
            ...
            List<IDictionary<string, object>> adList, List<IDictionary<string, object>> adList2)
        {
            ...
            if (insert_success && j == adList.Count && j2 == adList2.Count)
            {
                ...
                try
                {
                    ...
                    var result = SqlHelper.ExecuteNonQuery(DatabaseConfig.ConnectionString2_TH, CommandType.StoredProcedure, "VPA_INT_POST_MAIN", plist);
        ...
        public DataTable SearchDonHangDD_INY(DateTime ngayCt, string where0Ngay, string where1AM, string where2AD, string where3NhVt, string where4Dvcs, string where4Dvcs_2, string advance, out string loai_ct_chon)
        {
            if (where0Ngay.Length > 0) where0Ngay = "And " + where0Ngay;
            if (where1AM.Length > 0) where1AM = "And " + where1AM;
            ...
            if (where2AD.Length > 0 || where3NhVt.Length > 0 || where4Dvcs.Length > 0)
            {
                ...
                whereAD_Nhvt_Dvcs = string.Format("\n Where d.Stt_rec in (SELECT Stt_rec FROM " + AD_TableName + " WHERE Ma_ct = 'POH' {0} {2}"
                                     + (where3NhVt.Length == 0 ? "{3}" : "\n	And Ma_vt IN (SELECT Ma_vt FROM Alvt WHERE 1 = 1 {3})")
                                     + "\n		{4})"
                    , where0Ngay, "1", where2AD, where3NhVt, where4Dvcs);
            }
            ...
            var tbl = V6BusinessHelper.ExecuteProcedure("VPA_GET_STOCK_INT_INY", plist).Tables[0];
            return tbl;
        }
    }
}
