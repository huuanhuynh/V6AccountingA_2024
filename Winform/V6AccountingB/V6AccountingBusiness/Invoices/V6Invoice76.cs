    ...
    public class V6Invoice76 : V6InvoiceBase
    {
        ...
        public override bool InsertInvoice(IDictionary<string, object> amData, List<IDictionary<string, object>> adList)
        {
            ...
            else // insert không đủ dòng.
            {
                ...
                Logger.WriteToLog(string.Format("{0} Invoice76.InsertInvoice else.{1} {2}", V6Login.ClientName, stt_rec, V6Message));
            }
            return false;
        }
        ...
        public override bool InsertInvoice2_TH(IDictionary<string, object> amData, List<IDictionary<string, object>> adList)
        {
            ...
            if (insert_success && j == adList.Count)
            {
                ...
                try
                {
                    ...
                    var result = SqlHelper.ExecuteNonQuery(DatabaseConfig.ConnectionString2_TH, CommandType.StoredProcedure, "VPA_SOF_POST_MAIN", pList);
            ...
            IDictionary<string, object> keys)
        {
            ...
            if (insert_success && j == adList.Count)
            {
                ...
                try
                {
                    ...
                    SqlHelper.ExecuteNonQuery(DatabaseConfig.ConnectionString2_TH, CommandType.StoredProcedure, "VPA_SOF_POST_MAIN", pList);
                    return true;
                }
                catch (Exception ex)
                {
        ...
        public DataTable SearchPhieuXuat(string where0Ngay, string where1AM, string where2AD, string where3NhVt, string where4Dvcs, string where4Dvcs_2, out string loai_ct_chon)
        {
            //string template =
