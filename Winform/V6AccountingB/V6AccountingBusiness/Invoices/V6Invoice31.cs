...
{
    /// <summary>
    /// AP1: Hóa đơn mua hàng dịch vụ
    /// </summary>
    public class V6Invoice31 : V6InvoiceBase
    {
        public V6Invoice31() : base("AP1") { }
            ...
            List<IDictionary<string, object>> adList3)
        {
            ...
            if (insert_success && j == adList.Count && j2 == adList2.Count && j3 == adList3.Count)
            {
                ...
                try
                {
                    ...
                    V6BusinessHelper.ExecuteProcedureNoneQuery("VPA_AP1_POST_MAIN", pList);
            ...
            else // insert không đủ dòng.
            {
                ...
                Logger.WriteToLog(string.Format("{0} Invoice31.InsertInvoice else.{1} {2}", V6Login.ClientName, stt_rec, V6Message));
            }
            return false;
        }
            ...
            List<IDictionary<string, object>> adList3)
        {
            ...
            int j = 0, j2 = 0, j3 = 0;
            var insert_am_sql = SqlGenerator.GenInsertAMSql(V6Login.UserId, AMStruct, amData);
            SqlTransaction TRANSACTION2_TH = SqlConnect.CreateSqlTransaction2_TH(AM_TableName);
            ...
            try
            {
                ...
                var deleteAdSql = SqlGenerator.GenDeleteSql(ADStruct, keys);
                SqlConnect.ExecuteNonQuery(TRANSACTION2_TH, CommandType.Text, deleteAdSql);
                //Delete AD2_TableName
                var deleteAd2Sql = SqlGenerator.GenDeleteSql(AD2Struct, keys);
                SqlConnect.ExecuteNonQuery(TRANSACTION2_TH, CommandType.Text, deleteAd2Sql);
                //Delete AD3
                var deleteAd3Sql = SqlGenerator.GenDeleteSql(AD3Struct, keys);
                SqlConnect.ExecuteNonQuery(TRANSACTION2_TH, CommandType.Text, deleteAd3Sql);
                //Delete AM
                var deleteAMSql = SqlGenerator.GenDeleteSql(AMStruct, keys);
                SqlConnect.ExecuteNonQuery(TRANSACTION2_TH, CommandType.Text, deleteAMSql);

                insert_success = SqlConnect.ExecuteNonQuery(TRANSACTION2_TH, CommandType.Text, insert_am_sql) > 0;
                var currentMethodName = MethodBase.GetCurrentMethod().Name;
                j = InsertADlist(currentMethodName, TRANSACTION2_TH, adList, true);
                j2 = InsertAD2list(currentMethodName, TRANSACTION2_TH, adList2, true);
                j3 = InsertAD3list(currentMethodName, TRANSACTION2_TH, adList3, true);
            }
            catch (Exception ex)
            {
                #region === Rollback ===
            ...
            if (insert_success && j == adList.Count && j2 == adList2.Count && j3 == adList3.Count)
            {
                ...
                try
                {
                    ...
                    var result = SqlHelper.ExecuteNonQuery(DatabaseConfig.ConnectionString2_TH, CommandType.StoredProcedure, "VPA_AP1_POST_MAIN", pList);
        ...
        /// <summary>
        /// Cập nhập sửa thông tin chứng từ.
        /// </summary>
        /// <param name="amData">Thông tin chính</param>
        /// <param name="adList">Chi tiết 1</param>
        /// <param name="adList2">Chi tiết 2</param>
        /// <param name="adList3">Chi tiết 3</param>
            ...
            IDictionary<string, object> keys)
        {
            ...
            if (insert_success && j == adList.Count && j2 == adList2.Count && j3 == adList3.Count)
            {
                ...
                try
                {
                    ...
                    SqlHelper.ExecuteNonQuery(DatabaseConfig.ConnectionString2_TH, CommandType.StoredProcedure, "VPA_AP1_POST_MAIN", pList);
        ...
        public override bool DeleteInvoice(string sttrec)
        {
            try
            {
                ...
                V6BusinessHelper.ExecuteProcedureNoneQuery("VPA_AP1_DELETE_MAIN", plist);
                return true;
            }
            catch (Exception ex)
            {
