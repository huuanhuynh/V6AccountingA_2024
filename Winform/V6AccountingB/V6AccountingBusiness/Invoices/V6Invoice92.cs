    ...
    public class V6Invoice92: V6InvoiceBase
    {
            ...
            List<IDictionary<string, object>> adList, List<IDictionary<string, object>> adList2)
        {
            ...
            else // insert không đủ dòng.
            {
                ...
                Logger.WriteToLog(string.Format("{0} Invoice92.InsertInvoice else.{1} {2}", V6Login.ClientName, stt_rec, V6Message));
            }
            return false;
        }
            ...
            List<IDictionary<string, object>> adList, List<IDictionary<string, object>> adList2)
        {
            ...
            if (insert_success && j == adList.Count && j2 == adList2.Count)
            {
                ...
                try
                {
                    ...
                    var result = SqlHelper.ExecuteNonQuery(DatabaseConfig.ConnectionString2_TH, CommandType.StoredProcedure, "VPA_POH_POST_MAIN", plist);
