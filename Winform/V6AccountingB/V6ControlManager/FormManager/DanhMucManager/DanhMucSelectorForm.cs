...
{
    public partial class DanhMucSelectorForm : V6Form
    {
        public DanhMucSelectorForm()
        {
            InitializeComponent();
        }

        public DanhMucSelectorForm(AldmConfig config, string filter)
        {
            InitializeComponent();
            _aldmConfig = config;
            _filter = filter;
        ...
        private const string KEY_FIELD = "MA_VT"; // DUNG CHECK TRUNG MA
        ...
        private void MyInit()
        {
            try
            {
                _dmv = new DanhMucView("ITEMID", "Chọn vật tư", "ALVT", _filter, "MA_VT", _aldmConfig);
        ...
        private void CopyRowLeftToRight(IDictionary<string, object> rowData)
        {
            ...
            string ma_vt_value = rowData[KEY_FIELD].ToString().Trim();
            // Check
            if (!chkChonTrung.Checked)
            {
                foreach (DataRow row0 in _targetTable.Rows)
                {
                    if (row0[KEY_FIELD].ToString().Trim().ToUpper() == ma_vt_value.ToUpper()) return;
                }
            }
        ...
        private void btnAddSelect_Click(object sender, EventArgs e)
        {
            ...
            foreach (KeyValuePair<int, DataGridViewRow> item in rowDic)
            {
                var rowData = item.Value.ToDataDictionary();
                if (rowData.ContainsKey("UID")) rowData.Remove("UID");
                CopyRowLeftToRight(rowData);
            }
        }
        ...
        public string GetFieldsString()
        {
            ...
            {
                result += ";" + row.Cells[KEY_FIELD].Value.ToString().Trim();
            }
            if (result.Length > 0) result = result.Substring(1);
            return result;
        }
        
        public DataTable _targetTable;
        DataTable GenTable2Struct()
        {
            //
            _targetTable = (_dmv.dataGridView1.DataSource as DataTable).Clone();// new DataTable("GenFieldInfoList");
            ...
            ////if(_aldmConfig.Mact != "IXB") _targetTable.Columns.Add("MA_KHO_I", typeof(string));
        ...
        private void DanhMucSelectorForm_SizeChanged(object sender, EventArgs e)
        {
            FixSize();
        }
        ...
        private void dataGridView2_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            //        return;
            //    }
            //    var listFieldCanReplace = ObjectAndString.SplitString(_aldmConfig.EXTRA_INFOR["CT_REPLACE"].ToUpper());
