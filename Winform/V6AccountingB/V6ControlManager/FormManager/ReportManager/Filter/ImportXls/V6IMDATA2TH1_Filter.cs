...
namespace V6ControlManager.FormManager.ReportManager.Filter
{
    public partial class V6IMDATA2TH1_Filter: FilterBase
    {
        ...
        private void btnCheckConnection_Click(object sender, EventArgs e)
        {
            try
            {   
                var date = (DateTime)SqlHelper.ExecuteScalar(DatabaseConfig.ConnectionString2_TH, CommandType.Text, "Select GETDATE()");
                this.ShowInfoMessage("Kết nối ổn.\nGiờ server 2_TH: " + ObjectAndString.ObjectToString(date, "dd/MM/yyyy HH:mm:ss"));
            }
            catch (Exception ex)
            {
                this.ShowInfoMessage("Không kết nối được server 2_TH\n" + ex.Message);
            }
        }
    }
}
