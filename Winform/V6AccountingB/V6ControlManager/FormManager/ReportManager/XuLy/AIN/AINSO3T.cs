...
namespace V6ControlManager.FormManager.ReportManager.XuLy
{
    public class AINSO3T : XuLyBase
    {
        ...
        {
            string status = "";
            if (FilterControl == null) return string.Format("F5: {0}, F6: {1}, F7: {2}, F9: {3}, F10: {4}",
                    V6Text.Text("CHITIET"), V6Text.Text("XUATEXCEL_NHIEUFILE"), V6Text.Text("XUATEXCEL_NHIEUSHEET"), V6Text.Text("INTUNGTRANG"), V6Text.Text("INLIENTUC"));
            if (FilterControl.F6) status += ", F5: " + V6Text.Text("CHITIET");
        ...
        private void F6Thread()
        {
            ...
            while (i < dataGridView1.Rows.Count)
            {
                ...
                try
                {
                    if (row.IsSelect())
                    {
                        var MA_KH = (row.Cells[KEYFIELD].Value ?? "").ToString().Trim();
                        ...
                        if (MenuButton.UseXtraReport != shift_is_down)
                        {
                            ...
                            view.AutoExportExcelFileName = All_Objects["saveFile"] + "_" + MA_KH + All_Objects["saveExt"];
                        ...
                        else
                        {
                            ...
                            view.AutoExportExcel = All_Objects["saveFile"] + "_" + MA_KH + ".xls";
