...
{
    public partial class ARCMO_F3F4 : V6FormControl
    {
        ...
        public ARCMO_F3F4(V6Mode mode, string stt_rec, SortedDictionary<string, object> data)
        {
            _mode = mode;
            _stt_rec = stt_rec;
            if (mode == V6Mode.Edit)
        ...
        private void MyInit()
        {
            try
            {
                ...
                var length = V6BusinessHelper.VFV_iFsize("ARCMO", "SO_CT");
                if (length == 0) length = 12;
                txtso_ct.MaxLength = length;
                length = V6BusinessHelper.VFV_iFsize("ARCMO", "dien_giai");
