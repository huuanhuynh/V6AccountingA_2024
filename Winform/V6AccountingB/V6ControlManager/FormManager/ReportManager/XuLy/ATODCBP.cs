...
{
    public class ATODCBP : XuLyBase
    {
        public ATODCBP(string itemId, string program, string reportProcedure, string reportFile, string reportCaption, string reportCaption2)
        ...
        {
            V6ControlFormHelper.SetStatusText2(string.Format("F3: {0},F4: {1}, F8: {2}", V6Text.Text("SUA"), V6Text.Text("TTGTCCDC"), V6Text.Text("XTGTCCDC")));
        }
        ...
        public override void FormatGridViewExtern()
        {
            try
            {
                V6ControlFormHelper.FormatGridView(dataGridView1, "CC0", "=", 1, true, true, Color.MediumAquamarine);
            }
            catch (Exception ex)
            {
                            ...
                            && dataGridView1.Columns.Contains("UID"))
                        {
                            ...
                            if (!string.IsNullOrEmpty(selectedSttRec) && !string.IsNullOrEmpty(selectedMaCt))
                            {
                                ...
                                var plist = new List<SqlParameter>
                                    ...
                                    var am = V6BusinessHelper.Select("ADBPCC", "*",
                                        "UID=@uid",
                                        "", "", plist.ToArray()).Data;

                              

                                    var fText = "Sửa điều chuyển bộ phận sử dụng: " + sothets;
                                    var f = new V6Form
                                    {
                                        Text = fText,
                                        AutoSize = true,
                                    ...
                                    var hoaDonForm = new ATODCBP_F3(selectedSttRec, am.Rows[0]);
                    ...
                    if (dataGridView1.Columns.Contains("Stt_rec") && dataGridView1.Columns.Contains("Ma_ct"))
                    {
                        ...
                        if (!string.IsNullOrEmpty(selectedSttRec) && !string.IsNullOrEmpty(selectedMaCt))
                        {
                            ...
                            if (amName != "")
                            {
                                ...
                                var fText = "Thêm điều chuyển bộ phận sử dụng: " + sothets;
                                var f = new V6Form
                                {
                                    Text = fText,
                                    AutoSize = true,
                                ...
                                var hoaDonForm = new ATODCBP_F4(selectedSttRec, am.Rows[0]);
