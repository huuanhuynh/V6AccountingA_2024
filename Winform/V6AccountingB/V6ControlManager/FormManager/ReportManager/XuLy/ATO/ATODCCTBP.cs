...
{
    public class ATODCCTBP : XuLyBase
    {
        public ATODCCTBP(string itemId, string program, string reportProcedure, string reportFile, string reportCaption, string reportCaption2)
        ...
        public override void SetStatus2Text()
        {
            ...
            {
                text = string.Format("F3: {0}, F4: {1}, F8: {2}", V6Text.Text("SUA"), V6Text.Text("TTLKHCC"), V6Text.Text("XTLCC"));
            }
        ...
        public override void FormatGridViewExtern()
        {
            try
            {
                V6ControlFormHelper.FormatGridView(dataGridView1, "CC0", "=", 1, true, false, Color.MediumAquamarine);
            }
            catch (Exception ex)
            {
                    ...
                    else
                    {
                            ...
                            && dataGridView1.Columns.Contains("UID"))
                        {
                            ...
                            if (!string.IsNullOrEmpty(selectedSttRec) && !string.IsNullOrEmpty(selectedMaCt))
                            {
                                ...
                                var am = V6BusinessHelper.Select("ADCTCCBP", "*",
                                    "UID=@uid",
                                    "", "", plist.ToArray()).Data;



                                var fText = "Sửa tỷ lệ phân bổ : " + sothets;
                                var f = new V6Form
                                {
                                    Text = fText,
                                    AutoSize = true,
                                ...
                                var hoaDonForm = new ATODCCTBP_F3(selectedSttRec, am.Rows[0]);
                    ...
                    if (dataGridView1.Columns.Contains("Stt_rec") && dataGridView1.Columns.Contains("Ma_ct"))
                    {
                        ...
                        if (!string.IsNullOrEmpty(selectedSttRec) && !string.IsNullOrEmpty(selectedMaCt))
                        {
                            ...
                            if (amName != "")
                            {
                                ...
                                var fText = "Thêm tỷ lệ phân bổ : " + sothets;
                                var f = new V6Form
                                {
                                    Text = fText,
                                    AutoSize = true,
                                ...
                                var hoaDonForm = new ATODCCTBP_F4(selectedSttRec, am.Rows[0]);
