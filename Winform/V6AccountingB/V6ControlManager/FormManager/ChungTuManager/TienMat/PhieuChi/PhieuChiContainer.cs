...
{
    public partial class PhieuChiContainer : V6Control
    {
        private Timer _timerHideMessage;
        private int _timeCount, currentTabIndex = -1;
        private string _mact;
        private V6Invoice51 invoice;
        public PhieuChiContainer()
        {
            m_itemId = "itemId";
            invoice = new V6Invoice51();
        ...
        public PhieuChiContainer(string mact, string itemId)
        {
            _mact = mact;
            m_itemId = itemId;
            invoice = new V6Invoice51(mact);
            if (invoice.Alct == null || invoice.Alct.Rows.Count == 0)
            {
                this.ShowWarningMessage("Kiểm tra mã chứng từ!");
                Hide();
        ...
        private void AddTab()
        {
            try
            {
                var hoadon = new PhieuChiControl(_mact, m_itemId){ Dock = DockStyle.Fill };
            ...
            {
                this.ShowErrorMessage(GetType() + ".AddTab: " +ex.Message, "PhieuChi");
            }
        }
