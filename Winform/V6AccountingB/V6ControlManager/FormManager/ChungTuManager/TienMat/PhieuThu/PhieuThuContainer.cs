...
{
    public partial class PhieuThuContainer : V6Control
    {
        private Timer _timerHideMessage;
        private int _timeCount, currentTabIndex = -1;
        private readonly string _mact;
        private readonly V6Invoice41 invoice;
        public PhieuThuContainer()
        {
            m_itemId = "itemId";
            invoice = new V6Invoice41();
        ...
        public PhieuThuContainer(string mact, string itemId)
        {
            _mact = mact;
            m_itemId = itemId;
            invoice = new V6Invoice41(mact);
            if (invoice.Alct == null || invoice.Alct.Rows.Count == 0)
            {
                this.ShowWarningMessage("Kiểm tra mã chứng từ!");
                Hide();
        ...
        private void AddTab()
        {
            try
            {
                var hoadon = new PhieuThuControl(_mact, m_itemId) { Dock = DockStyle.Fill };
                var tab = new TabPage(invoice.Name);
            ...
            {
                this.ShowErrorMessage(GetType() + ".AddTab: " +ex.Message, "PhieuThu");
            }
        }
