...
namespace V6ControlManager.FormManager.ChungTuManager.Filter
{
    ...
    /// </summary>
    public partial class FilterView_ARSODU0_Cont : Form
    {
        ...
        public FilterView_ARSODU0_Cont(V6InvoiceBase invoice, V6ColorTextBox sender, string sttRec, string maDvcs, string initStrFilter)
        {
            InitializeComponent();
        ...
        private void Init()
        {
            try
            {
                var aldm_data = V6BusinessHelper.Select("aldm", "GRDS_V1,GRDF_V1,GRDHV_V1,GRDHE_V1,Title,Title2", "ma_dm='" + "ARS82" + "'").Data;
            ...
            {
                this.ShowErrorMessage(GetType() + ".Init : " + ex.Message, "ChungTuManager FilterView_ARSODU0_Cont");
            }
        }
        ...
        private void FormatDataGridView()
        {
            try
            {
                ...
                dataGridView1.ShowColumnsAldm("ARS82");
                dataGridView1.HideColumnsAldm("ARS82");
        ...
        private void SearchData()
        {
            ...
            try
            {
                var alct0 = Invoice.GetCuocCont_All_Cust(_sttRec, txtMaDVCS.Text.Trim(), v6ColorDateTimePick1.Value, v6ColorDateTimePick2.Value, _filterString);
        ...
        private string GenFilterString()
        {
            ...
            try
            {
                ...
                {
                    result += string.Format(" and a.{0}", TxtMa_bp.Query);
                }
                if (txtMaKh.IsSelected)
                {
                    result += string.Format(" and a.{0}", txtMaKh.QueryCheck);
                }
                if (taiKhoan.IsSelected)
                {
                    result += string.Format(" and a.{0}", taiKhoan.QueryCheck);
                }
                if (Txtma_nvien.Text.Trim() != "")
                {
                    result += string.Format(" and a.{0}", Txtma_nvien.Query);
                }
                ...
                if (cacNhomKhachQuery.Length > 4)
                {
                    ...
                    where_nhkh = string.Format("a.Ma_kh in (Select ma_kh from Alkh where {0})", cacNhomKhachQuery);
                }
                if (where_nhkh != "")
                {
