...
{
    public partial class PhieuXuatDieuChuyenContainer : V6Control
    {
        private Timer _timerHideMessage;
        private int _timeCount;
        private int currentTabIndex = -1;
        public PhieuXuatDieuChuyenContainer()
        {
        ...
        public PhieuXuatDieuChuyenContainer(string itemId)
        {
            m_itemId = itemId;
            V6Invoice85 invoice = new V6Invoice85();
            if (invoice.Alct == null || invoice.Alct.Rows.Count == 0)
        ...
        private void AddTab()
        {
            try
            {
                var PhieuXuatDieuChuyen = new PhieuXuatDieuChuyenControl(m_itemId) { Dock = DockStyle.Fill };
                
                var tab = new TabPage(V6Text.Invoice);
                tab.Controls.Add(PhieuXuatDieuChuyen);
                PhieuXuatDieuChuyen.Disposed += delegate {
                    tab.Dispose();
                };
            ...
            {
                this.ShowErrorMessage(GetType() + ".AddTab: " +ex.Message, "PhieuXuatDieuChuyen");
            }
        }
        ...
        private void btnThemPhieuXuatDieuChuyen_Click(object sender, EventArgs e)
        {
            AddTab();
        }
