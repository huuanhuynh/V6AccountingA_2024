...
namespace V6ControlManager.FormManager.ChungTuManager.TonKho.PhieuXuatDieuChuyen
{
    public partial class PhieuXuatDieuChuyenControl : V6InvoiceControl
    {
        ...
        /// </summary>
        public PhieuXuatDieuChuyenControl(string itemId, string sttRec)
        {
            m_itemId = itemId;
            InitializeComponent();
            MyInit();
        ...
        private V6VvarTextBox _maVt, _dvt1, _maKho2, _Ma_nx_i,  _tkVt, _maLo;
        ...
        void MaKho_V6LostFocus(object sender)
        {
            XuLyChonMaKho();
        }
        ...
        private void GetTon13()
        {
                        ...
                        for (int i = _dataLoDate.Rows.Count - 1; i >= 0; i--)
                        {
                            ...
                            if (maVt == data_maVt && maKhoI == data_maKhoI)
                            {
                                ...
                                foreach (DataRow row in AD.Rows) //Duyet qua cac dong chi tiet
                                {
                                    string c_sttRec0 = row["Stt_rec0"].ToString().Trim();
                                    string c_maVt = row["Ma_vt"].ToString().Trim().ToUpper();
                                    string c_maKhoI = txtMaKhoX.Text.Trim().ToUpper();// row["Ma_kho_i"].ToString().Trim().ToUpper();
            ...
            {
                this.ShowErrorMessage(string.Format("{0} {1} {2} {3}", V6Login.ClientName, GetType(), MethodBase.GetCurrentMethod().Name, _sttRec, ex.Message));
            }
        }
        ...
        private void Tim()
        {
            try
            {
                if (V6Login.UserRight.AllowView("", Invoice.CodeMact))
                {
                    ...
                    if (_timForm == null)
                        _timForm = new TimPhieuXuatDieuChuyenForm(this);
        ...
        private void XuLyDetailClickAdd()
        {
            ...
            {
                this.ShowErrorMessage(string.Format("{0} {1}.{2} {3} {4}", V6Login.ClientName, GetType(), MethodBase.GetCurrentMethod().Name, _sttRec, ex.Message));
            }
        }
