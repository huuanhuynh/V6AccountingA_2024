...
{
    public partial class PhieuNhapKhoContainer : V6Control
    {
        private Timer _timerHideMessage;
        private int _timeCount, currentTabIndex = -1;
        public PhieuNhapKhoContainer()
        {
            m_itemId = "itemId";
        ...
        public PhieuNhapKhoContainer(string itemId)
        {
            m_itemId = itemId;
            V6Invoice74 invoice = new V6Invoice74();
            if (invoice.Alct == null || invoice.Alct.Rows.Count == 0)
        ...
        private void AddTab()
        {
            try
            {
                var PhieuNhapKho = new PhieuNhapKhoControl(m_itemId) { Dock = DockStyle.Fill };
                
                var tab = new TabPage(V6Text.Invoice);
                tab.Controls.Add(PhieuNhapKho);
                PhieuNhapKho.Disposed += delegate {
                    tab.Dispose();
                };
            ...
            {
                this.ShowErrorMessage(GetType() + ".AddTab: " +ex.Message, "PhieuNhapKho");
            }
        }
