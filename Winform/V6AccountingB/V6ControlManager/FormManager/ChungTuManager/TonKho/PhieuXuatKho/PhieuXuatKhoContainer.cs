...
{
    public partial class PhieuXuatKhoContainer : V6Control
    {
        private Timer _timerHideMessage;
        private int _timeCount;
        private int currentTabIndex = -1;
        public PhieuXuatKhoContainer()
        {
        ...
        public PhieuXuatKhoContainer(string itemId)
        {
            m_itemId = itemId;
            V6Invoice84 invoice = new V6Invoice84();
            if (invoice.Alct == null || invoice.Alct.Rows.Count == 0)
        ...
        private void AddTab()
        {
            try
            {
                var PhieuXuatKho = new PhieuXuatKhoControl(m_itemId) { Dock = DockStyle.Fill };
                
                var tab = new TabPage(V6Text.Invoice);
                tab.Controls.Add(PhieuXuatKho);
                PhieuXuatKho.Disposed += delegate {
                    tab.Dispose();
                };
            ...
            {
                this.ShowErrorMessage(GetType() + ".AddTab: " +ex.Message, "PhieuXuatKho");
            }
        }
        ...
        private void btnThemPhieuXuatKho_Click(object sender, EventArgs e)
        {
            AddTab();
        }
