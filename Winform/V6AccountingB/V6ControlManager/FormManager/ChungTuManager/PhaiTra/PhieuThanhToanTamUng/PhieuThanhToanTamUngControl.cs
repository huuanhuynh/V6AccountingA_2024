...
namespace V6ControlManager.FormManager.ChungTuManager.PhaiTra.PhieuThanhToanTamUng
{
    public partial class PhieuThanhToanTamUngControl : V6InvoiceControl
    {
        ...
        /// </summary>
        public PhieuThanhToanTamUngControl(string itemId, string sttRec)
        {
            m_itemId = itemId;
            InitializeComponent();
            MyInit();
        ...
        private void XuLyThayDoiTyGia()
        {
            try
            {
                ...
                foreach (DataRow row in AD.Rows)
                {
                    ...
                    temp = ObjectAndString.ObjectToDecimal(row["GIA_NT"]);
                    if (temp != 0)
                        row["GIA"] = V6BusinessHelper.Vround(temp * ty_gia, M_ROUND_GIA);
