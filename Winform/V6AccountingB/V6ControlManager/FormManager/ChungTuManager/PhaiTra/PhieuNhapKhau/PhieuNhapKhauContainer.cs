...
{
    public partial class PhieuNhapKhauContainer : V6Control
    {
        private Timer _timerHideMessage;
        private int _timeCount, currentTabIndex = -1;
        public PhieuNhapKhauContainer()
        {
        ...
        public PhieuNhapKhauContainer(string itemId)
        {
            m_itemId = itemId;
            V6Invoice72 invoice = new V6Invoice72();
            if (invoice.Alct == null || invoice.Alct.Rows.Count == 0)
        ...
        private void AddTab()
        {
            try
            {
                var hoadon = new PhieuNhapKhauControl(m_itemId){ Dock = DockStyle.Fill };
            ...
            {
                this.ShowErrorMessage(GetType() + ".POB AddTab: " +ex.Message, "PhieuNhapKhau");
            }
        }
