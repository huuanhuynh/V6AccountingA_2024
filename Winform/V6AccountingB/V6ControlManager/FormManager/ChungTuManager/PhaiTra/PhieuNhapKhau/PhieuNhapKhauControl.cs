...
using V6ControlManager.FormManager.ChungTuManager.PhaiTra.PhieuNhapKhau.ChonDeNghiNhap;
...
namespace V6ControlManager.FormManager.ChungTuManager.PhaiTra.PhieuNhapKhau
{
    public partial class PhieuNhapKhauControl : V6InvoiceControl
    {
        ...
        public V6VvarTextBox _maVt, _Ma_lnx_i, _dvt1, _maKho, _maKhoI, _maKho2, _tkVt, _maLo, _ma_thue_i, _maViTri, _maViTri2;
        ...
        public V6NumberTextBox _ton13, _ton13s, _ton13Qd, _tienNt, _tien, _nk_Nt, _nk, _ts_nk, _sl_qd, _sl_qd2, _hs_qd1, _hs_qd2,
        ...
        public V6NumberTextBox _t_tien22, _t_tien_nt22, _thue_suat22, _t_thue22, _t_thue_nt22, _tien_cn_nt22, _tien_cn22, _han_tt22;
            ...
            foreach (KeyValuePair<string, AlctControls> item in detailControlList1)
            {
                ...
                switch (NAME)
                {
                    ...
                    case "HSD":
                        ...
                        break;
                    case "NK_NT":
                        _nk_Nt = (V6NumberTextBox)control;
                        _nk_Nt.V6LostFocus += delegate
                        {
                            TinhTienNt();
                            Tinh_thue_ct();
                        };
        ...
        public V6NumberTextBox _PsNoNt_33, _PsCoNt_33, _PsNo_33, _PsCo_33,
        ...
        public void TinhTienNt_row(DataGridViewRow row)
        {
            try
            {
                row.Cells["TIEN_NT"].Value = ObjectAndString.ObjectToDecimal(row.Cells["TIEN_NT0"].Value) + ObjectAndString.ObjectToDecimal(row.Cells["CP_NT"].Value)
                                             ...
                                             + ObjectAndString.ObjectToDecimal(row.Cells["NK_NT"].Value);
                if (_maNt == _mMaNt0)
                {
                    row.Cells["TIEN0"].Value = row.Cells["TIEN_NT0"].Value;
                ...
                else
                {
                    row.Cells["TIEN"].Value =  ObjectAndString.ObjectToDecimal(row.Cells["TIEN0"].Value) + ObjectAndString.ObjectToDecimal(row.Cells["CP"].Value)
                                               ...
                                               + ObjectAndString.ObjectToDecimal(row.Cells["NK"].Value);
                }
            }
            catch (Exception ex)
            {
                ...
                if (txtTongGiam.Value != 0)
                {
                    ...
                    for (var i = 0; i < AD.Rows.Count; i++)
                    {
                        ...
                        if (t_tien_nt0 != 0)
                        {
                            ...
                            var nk_nt = ObjectAndString.ObjectToDecimal(row_i["NK_NT"]);
                            var nk = ObjectAndString.ObjectToDecimal(row_i["NK"]);
                            ...
                            decimal tien = tien_nt0 + cp_nt - ck_nt - gg_nt + nk_nt;
                    ...
                    if (index_gg != -1)
                    {
                        ...
                        var nk_nt = ObjectAndString.ObjectToDecimal(row_gg["NK_NT"]);
                        var nk = ObjectAndString.ObjectToDecimal(row_gg["NK"]);
        ...
        public void TinhThueNhapKhau_row(DataGridViewRow row)
        {
            try
            {
                var cell_NK_NT = row.Cells["NK_NT"];
                var cell_NK = row.Cells["NK"];
                var cell_Tien_NT0 = row.Cells["TIEN_NT0"];
                var cell_TS_NK = row.Cells["TS_NK"];
                cell_NK_NT.Value = V6BusinessHelper.Vround(ObjectAndString.ObjectToDecimal(cell_Tien_NT0.Value) * ObjectAndString.ObjectToDecimal(cell_TS_NK.Value) / 100, M_ROUND_NT);
                if (_maNt == _mMaNt0)
                {
                    cell_NK.Value = cell_NK_NT.Value;
                }
                else
                {
                    cell_NK.Value = V6BusinessHelper.Vround(ObjectAndString.ObjectToDecimal(cell_NK_NT.Value) * txtTyGia.Value, M_ROUND);
                }
            }
            catch (Exception ex)
            {
        ...
        public void TinhLaiTienThueCT()
        {
            ...
            for (var i = 0; i < AD.Rows.Count; i++)
            {
                ...
                {
                    ck_value = ObjectAndString.ObjectToDecimal(row["CK"]);
                    cknt_value = ObjectAndString.ObjectToDecimal(row["CK_NT"]);
                }
                if (M_POA_VAT_WITH_CK_GG.Length > 1 && M_POA_VAT_WITH_CK_GG[1] == '1')
                {
                    gg_value = ObjectAndString.ObjectToDecimal(row["GG"]);
                    ggnt_value = ObjectAndString.ObjectToDecimal(row["GG_NT"]);
                }
        ...
        private TimPhieuNhapKhauForm SearchForm
        {
            get
            {
                if (_timForm == null || _timForm.IsDisposed)
        ...
        {
            FixMenuChucNangItemShiftText(chonTuExcelMenu, chonDonHangMuaMenu, chonDeNghiNhapMenu, importXmlMenu);
        }
