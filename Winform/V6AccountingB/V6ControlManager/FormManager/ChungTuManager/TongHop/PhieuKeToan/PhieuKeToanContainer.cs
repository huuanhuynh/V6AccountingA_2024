...
{
    public partial class PhieuKeToanContainer : V6Control
    {
        private Timer _timerHideMessage;
        private int _timeCount;
        private int currentTabIndex = -1;
        private string _mact;
        public PhieuKeToanContainer()
        {
        ...
        public PhieuKeToanContainer(string mact, string itemId)
        {
            _mact = mact;
            m_itemId = itemId;
            V6Invoice11 invoice = new V6Invoice11(mact);
            if (invoice.Alct == null || invoice.Alct.Rows.Count == 0)
            {
                this.ShowWarningMessage("Kiểm tra mã chứng từ!");
                Hide();
        ...
        private void AddTab()
        {
            try
            {
                var hoadon = new PhieuKeToanControl(_mact, m_itemId){ Dock = DockStyle.Fill };
            ...
            {
                this.ShowErrorMessage(GetType() + ".AddTab: " +ex.Message, "PhieuKeToan");
            }
        }
