...
{
    public partial class CPX_HoaDonDichVuCoSLForm : V6Form
    {
        V6Invoice84 Invoice = new V6Invoice84();
        //private readonly HoaDonDichVuCoSLControl _PhieuNhapMuaForm;
        private CPX_HoaDonDichVuCoSLKetQua _locKetQua;
        ...
        public CPX_HoaDonDichVuCoSLForm()
        {
            InitializeComponent();
        }

        public CPX_HoaDonDichVuCoSLForm(DateTime ngayCt, string ma_dvcs, string ma_kh)
        {
            InitializeComponent();
        ...
        private void DoSearch()
        {
            try
            {
                tAM = Invoice.SearchPhieuXuat_HoaDon(_ngayCt, _where0Time, _where1AM, _where2AD, _w3NhomVt, _w4Dvcs, _w4Dvcs_2, _advance, out _loai_ct_chon);
            ...
            string oper = "=", bool and = true)
        {
            ...
            if (soTienTu.Value != 0)
            {
                ...
                {
                    result += (result.Length > 0 ? and_or : "")
                              + tbL + "T_TIEN_NT >=" + soTienTu.Value.ToString(CultureInfo.InvariantCulture);

                    result += (result.Length > 0 ? and_or : "")
                              + tbL + "T_TIEN_NT <=" + soTienDen.Value.ToString(CultureInfo.InvariantCulture);
                }
                else
                {
                    result += (result.Length > 0 ? and_or : "")
