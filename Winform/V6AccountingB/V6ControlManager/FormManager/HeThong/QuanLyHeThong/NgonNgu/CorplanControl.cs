            ...
            try
            {
                ...
                f.InsertSuccessEvent += dic =>
                {
                    try
                    {
                        ...
                        txtID.ReadOnly = true;
                        txtID.TabStop = false;
                        txtID.Text = dic[ID_FIELD].ToString().Trim();
                        txtID.GotFocus += (sender, e) =>
                        {
                            _currentRowData = dic;
                            CopyCurrentRowIdToClipBoard();
                        };
                        txtID.KeyPress += (sender, e) =>
                        {
                            if (char.IsLetterOrDigit(e.KeyChar))
                            {
                                foreach (KeyValuePair<string, LineObjects> item in lineDic)
                                {
                                    TextBox txt_default = item.Value.TxtDefault;
                                    TextBox txt_id = item.Value.TxtID;
                                    string KEY_CHAR = e.KeyChar.ToString().ToUpper();
                                    if (string.CompareOrdinal(txt_id.Text, txtID.Text) > 0 && txt_default.Text.ToUpper().StartsWith(KEY_CHAR))
                                    {
                                        txt_id.Focus();
                                        break;
                                    }
                        ...
                        check_ChangeV.AutoSize = true;
                        check_ChangeV.Text = " ";
                        check_ChangeV.Checked = dic.ContainsKey("CHANGE_V") &&
                                                ObjectAndString.ObjectToBool(dic["CHANGE_V"]);
                        check_ChangeV.GotFocus += (sender, e) =>
                        {
                            _currentRowData = dic;
                            CopyCurrentRowIdToClipBoard();
                        ...
                        check_ChangeV.CheckedChanged += (sender, args) =>
                        {
                            dic["CHANGE_V"] = check_ChangeV.Checked ? "1" : "0";
                            b.Enabled = true;
                        };
                        lineO.ChkChangeV = check_ChangeV;
                        panel1.Controls.Add(check_ChangeV);
                        toolTipV6FormControl.SetToolTip(check_ChangeV,
                            V6Setting.IsVietnamese ? "Dịch tiếng Việt" : "Translate Vietnamese");
