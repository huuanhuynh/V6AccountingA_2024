...
namespace V6Init
{
    ...
    public class AldmConfig : Config
    {
        ...
        /// </summary>
        public string GRDH_LANG_V1 { get { return true ? GRDHV_V1 : GRDHE_V1; } }
    ...
    public class XtbConfig : Config
    {
        ...
        public XtbConfig(string dir, IDictionary<string, object> data)
            : base(data)
        {
            __dir = dir;
        }
        ...
        public string sqlxmlfilename1
        {
            get
            {
                string path = GetString("sqlxml1");
                if (!Path.IsPathRooted(path)) path = Path.Combine(__dir, path);
                return path;
            }
        }

        public string sqlxmlfilename2
        {
            get
            {
                string path = GetString("sqlxml2");
                if (!Path.IsPathRooted(path)) path = Path.Combine(__dir, path);
                return path;
            }
        }

        public string sqlxmlfilename3
        {
            get
            {
                string path = GetString("sqlxml3");
                if (!Path.IsPathRooted(path)) path = Path.Combine(__dir, path);
                return path;
            }
        }

        public string vpn_supfolder { get { return GetString("vpn_supfolder"); } }
        ...
        public string Ecommand { get { return GetString("Ecommand"); } }
        public string Type { get { return GetString("Type"); } }
        ...
        public string REFKEY { get { return GetString("REFKEY"); } }
    }

    public class ConfigManager
    {
        public static AlctConfig GetAlctConfig(string conString, string ma_ct)
        {
            AlctConfig config = new AlctConfig();
            try
            {
                SqlParameter[] plist = { new SqlParameter("@p", ma_ct) };
                var executeResult = Functions.Select(conString, "Alct", "*", "Ma_ct=@p", "", "", plist);

                if (executeResult != null && executeResult.Rows.Count > 0)
                {
                    var tbl = executeResult;
                    var row = tbl.Rows[0];
        ...
        public static AldmConfig GetAldmConfig(string conString, string ma_dm)
        {
            AldmConfig lstConfig = null;
            try
            {
                SqlParameter[] plist = { new SqlParameter("@p", ma_dm) };
                var executeResult = Functions.Select(conString, "Aldm", "*", "Ma_dm=@p", "", "", plist);
        ...
        public static AldmConfig GetAldmConfigByTableName(string conString, string table_name)
        {
            AldmConfig lstConfig = new AldmConfig();
            try
            {
                SqlParameter[] plist = { new SqlParameter("@p", table_name) };
                var executeResult = Functions.Select(conString, "Aldm", "*", "Table_name=@p", "", "", plist);
        ...
        public static AlreportConfig GetAlreportConfig(string conString, string ma_bc)
        {
            AlreportConfig lstConfig = new AlreportConfig();
            try
            {
                SqlParameter[] plist = { new SqlParameter("@p", ma_bc) };
                var executeResult = Functions.Select(conString, "ALREPORT", "*", "Ma_bc=@p", "", "", plist);
        ...
        /// <returns></returns>
        public static V6ValidConfig GetV6ValidConfig(string conString, string ma_ct, int attribute)
        {
            V6ValidConfig lstConfig = new V6ValidConfig();
            try
            {
                SqlParameter[] plist = { new SqlParameter("@p1", ma_ct), new SqlParameter("@p2", attribute) };
                var executeResult = Functions.Select(conString, "V6Valid", "*", "[ma_ct]=@p1 and [attribute]=@p2", "", "", plist);
        ...
        /// <returns></returns>
        public static V6ValidConfig GetV6ValidConfigDanhMuc(string conString, string tableName)
        {
            V6ValidConfig lstConfig = new V6ValidConfig();
            try
            {
                SqlParameter[] plist = { new SqlParameter("@p", tableName) };
                var executeResult = Functions.Select(conString, "V6Valid", "*", "attribute=3 and [TABLE_NAME]=@p", "", "", plist);
