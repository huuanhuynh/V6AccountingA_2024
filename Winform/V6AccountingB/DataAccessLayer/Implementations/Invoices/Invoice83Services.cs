...
{
    public class Invoice83Services : IInvoice83Services
    {
            ...
            string where0Ngay, string where1AM, string where2AD, string where3NhVt, string where4Dvcs)
        {
            string template =
                ...
                + "\n JOIN (SELECT Stt_rec FROM " + tableNameAM + " WHERE Ma_ct = '" + mact + "'"
            ...
            var p2Template =
                "\n--{0}{1}\nAnd Stt_rec in (SELECT Stt_rec FROM " + tableNameAD + " WHERE Ma_ct = '" + mact + "' {0} {2}"
