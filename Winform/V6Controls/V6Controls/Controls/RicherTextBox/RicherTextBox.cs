...
namespace V6Controls.Controls.RicherTextBox
{
    ...
    public partial class RicherTextBox : UserControl
    {
        ...
        [Category("Settings")]
        [Description("Ngôn ngữ form V hoặc E")]
        ...
        private void RicherTextBox_Load(object sender, EventArgs e)
        {
            ...
            foreach (FontFamily family in FontFamily.Families)
            {
                ...
                auto1.Add(family.Name);
            }
            tscmbFont.SelectedItem = "Microsoft Sans Serif";
            tscmbFont.AutoCompleteMode = AutoCompleteMode.SuggestAppend;
            tscmbFont.AutoCompleteCustomSource = auto1;
            tscmbFont.AutoCompleteSource = AutoCompleteSource.CustomSource;
        ...
        private void tsbtnInsertPicture_Click(object sender, EventArgs e)
        {
            ...
            {
                dlg.Title = V ? "Chèn hình ảnh" : "Insert image";
                dlg.DefaultExt = "jpg";
                dlg.Filter = V ? "Hình ảnh|*.jpg;*.jpeg;*.gif;*.png;*.bmp|Bitmap Files|*.bmp|JPEG Files|*.jpg|GIF Files|*.gif|Tất cả|*.*"
                    : "Images|*.jpg;*.jpeg;*.gif;*.png;*.bmp|Bitmap Files|*.bmp|JPEG Files|*.jpg|GIF Files|*.gif|All files|*.*";
                dlg.FilterIndex = 1;
                if (dlg.ShowDialog() == DialogResult.OK)
                {
                    try
                    ...
                    {
                        if (V) MessageBox.Show("Không chèn hình được.", "Lỗi", MessageBoxButtons.OK, MessageBoxIcon.Error);
                        else MessageBox.Show("Unable to insert image.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
            }
        }
